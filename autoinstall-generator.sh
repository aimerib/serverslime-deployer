...
function cleanup() {
        ...
        if [ -n "${tmpdir+x}" ]; then
                rm -rf "$tmpdir"
                rm "$original_iso"
                rm "${script_dir}/${ubuntu_gpg_key_id}.keyring"
...
function parse_params() {
        ...
        download_iso="ubuntu-21.10-live-server-amd64.iso"
...
sed -i -e 's/set timeout=30/set timeout=0/g' "$tmpdir/boot/grub/grub.cfg"
sed -i -e 's/set timeout=30/set timeout=0/g' "$tmpdir/boot/grub/loopback.cfg"
...
xorriso -as mkisofs \
-D -r -V "ubuntu-autoinstall-$today" \
-append_partition 2 28732ac11ff8d211ba4b00a0c93ec93b --interval:local_fs:2470124d-2478587d::"${source_iso}" \
-cache-inodes -J -l -joliet-long \
-b '/boot/grub/i386-pc/eltorito.img' \
-c '/boot.catalog' \
-no-emul-boot -boot-load-size 4 \
-boot-info-table -eltorito-alt-boot \
-e '--interval:appended_partition_2_start_617531s_size_8464d:all::' \
